...
from ai_judge import analyze_listing  # Import the AI analysis function
...
async def main():
    ...
    # Process new listings with AI evaluation
    if all_new_listings:
        logger.info(f"Found {len(all_new_listings)} new listings.")
        
        # Evaluate all listings with AI
        for listing in all_new_listings:
            try:
                ai_verdict = await analyze_listing(listing)
                logger.info(f"AI Verdict for {listing['title']}: {ai_verdict['verdict']} | Profit SEK: {ai_verdict['estimated_profit']} | Profit %: {ai_verdict['profit_percentage']} | Reason: {ai_verdict['reason']}")
